...
# Create/update a stable symlink 'pr_key' inside an MSP's keystore/
fix_keystore_name () {
  # $1 = absolute path to the MSP dir (contains keystore/, signcerts/, etc.)
  local MSPDIR="$1"
  local KDIR="${MSPDIR}/keystore"

  [ -d "${KDIR}" ] || die "keystore not found: ${KDIR}"

  # Pick the most recent key file in keystore (handles filenames with or without _sk)
  local KEYFILE
  KEYFILE="$(ls -1t "${KDIR}"/* 2>/dev/null | head -n1)" || true

  [ -n "${KEYFILE}" ] || die "No private key file found in ${KDIR}"

  # Create or update a stable alias
  ln -sf "$(basename "${KEYFILE}")" "${KDIR}/pr_key"
  echo "🔗 ${KDIR}/pr_key -> $(basename "${KEYFILE}")"
}
...
# Normalize (alias) Admin private keys so paths stay stable across runs
fix_keystore_name "${PWD}/organizations/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp"
fix_keystore_name "${PWD}/organizations/peerOrganizations/org2.example.com/users/Admin@org2.example.com/msp"
