...
/**
 * Target extra payload bytes to push total TX size ≈ 16 KB.
 * For typical Fabric overhead (1.5–2 KB), 14,000–14,500 extra bytes gives ≈16 KB final size.
 */
const EXTRA_BYTES = parseInt(process.env.EXTRA_BYTES || '14500', 10);

function padField(base, n) {
    if (n <= 0) return base;
    // Add a small delimiter then repeat a lightweight character (1 byte per char)
...
class CreateCarWorkload extends WorkloadModuleBase {
    ...
    async submitTransaction() {
        ...
        // Distribute padding (~14.5 KB total)
        ...
        // Optional: print payload size (for debugging)
