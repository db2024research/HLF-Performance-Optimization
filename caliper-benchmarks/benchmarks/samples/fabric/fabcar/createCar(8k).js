...
/**
 * Target extra payload bytes to push total TX size ≈ 8 KB.
 * For typical Fabric overhead (~1.3–1.5 KB),
 * ~6400–6700 extra bytes gives ~8 KB final size.
 */
const EXTRA_BYTES = parseInt(process.env.EXTRA_BYTES || '6500', 10);

function padField(base, n) {
    if (n <= 0) return base;
    return `${base}|${'x'.repeat(n)}`; // ASCII → 1 byte per character
}
...
class CreateCarWorkload extends WorkloadModuleBase {
    ...
    async submitTransaction() {
        ...
        // Distribute padding (~6.5 KB total)
