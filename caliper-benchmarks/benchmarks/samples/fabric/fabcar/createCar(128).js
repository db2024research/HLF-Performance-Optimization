...
/**
 * Target extra payload bytes to push total TX size ≈ 128 KB.
 * Fabric overhead (envelope, certs, signatures, RW-set) is ~1.5–2 KB, so
 * adding ~126,000 extra bytes to the payload gives roughly 128 KB per tx on-ledger.
 */
const EXTRA_BYTES = parseInt(process.env.EXTRA_BYTES || '126000', 10);

function padField(base, n) {
    if (n <= 0) return base;
    // Add a small delimiter then repeat a light char so 1 char = 1 byte in UTF-8
...
class CreateCarWorkload extends WorkloadModuleBase {
    ...
    async submitTransaction() {
        ...
        // Spread padding (~126 KB total) across non-key fields
